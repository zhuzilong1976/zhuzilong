...
Virtual knockout on single-cell gene regulatory networks is used to nominate genes, and the resulting "differentially regulated" (DR) list is read as the downstream consequence of losing a gene. We show that it is a structural property of the network instead. Across **3,195 knockouts in four human microglia networks**, and in **10 published networks** (2,591–9,970 genes; human and mouse), the DR set was contained in the knocked-out gene plus its **direct out-neighbours** in **99.87%** of cases; all four exceptions were knockouts with **outdegree 0**, where the knockout is a no-op yet returned up to 15 significant genes. List size is set by outdegree through dilution of each target's incoming weight: under the package's default χ² null, **96% of knockouts returned exactly one significant gene, the knocked-out gene itself**, whereas the empirical null returned **outdegree + 1** while weight was concentrated (198 genes at outdegree 197). Biologically irrelevant knockouts can therefore dominate: in the MS network two negative-control genes produced the two largest DR sets (188 and 146 genes). The rule is **tool-specific**: with CellOracle, which propagates perturbations iteratively, 44–78% of affected genes lay outside the direct target set. A disease-versus-control comparison must be read against a same-condition null: the MS-versus-control rate (7.25% of genes at *p* < 0.05) did not exceed a split-half null (5.65%; Fisher *p* = 0.22), and the null's top genes were equally plausible. Containment and the panel result replicated in all 20 cell-subsample replicates; magnitude-based outputs did not. We give four reporting requirements.
