...
# (67b) same two checkpoints, but for the larger 1,000-cell replicates in
# work/stability_large; writes into the same checkpoint directory with a
# "_large" suffix so the 800-cell results are untouched.
#
...
print(c3tab[, c("tag","rep","n_genes","n_null","n_candidate_fdr05",
      ...
      row.names = FALSE)
write.csv(c3tab, file.path(out_dir, "checkpoint_C3_empirical_null_by_replicate_large.csv"),
          row.names = FALSE)
...
write.csv(c4tab, file.path(out_dir, "checkpoint_C4_paired_condition_comparison_large.csv"),
          row.names = FALSE)
