...
Virtual knockout on single-cell gene regulatory networks is used to nominate genes, and the resulting "differentially regulated" (DR) list is read as the downstream consequence of losing a gene. We show it is a structural property of the network instead. Across **3,195 knockouts in four human microglia networks** and **10 published networks** (2,591–9,970 genes; human and mouse), the DR set was contained in the knocked-out gene plus its **direct out-neighbours** in **99.87%** of cases; all four exceptions were knockouts with **outdegree 0**, where the knockout is a no-op yet returned up to 15 significant genes. List size is set by the knockout's outdegree alone: under the package's default χ² null, **96% of knockouts returned exactly one significant gene, the knocked-out gene itself**, whereas the empirical null called **every direct target** of low-outdegree knockouts (198 genes at outdegree 197) and none above a network-specific threshold (per-target share: rank AUC 0.50). Biologically irrelevant knockouts can therefore dominate: in the MS network two negative-control genes produced the two largest DR sets (188 and 146 genes). The rule is **tool-specific**: with CellOracle, which propagates iteratively, 44–78% of affected genes lay outside the direct target set. A disease-versus-control comparison must be read against a same-condition null: the MS-versus-control rate (7.25% at *p* < 0.05) did not exceed a split-half null (5.65%; Fisher *p* = 0.22), and the null's top genes were equally plausible. Containment and the panel result replicated in all 20 cell-subsample replicates; magnitude-based outputs did not. We give four reporting requirements.
...
Inferring gene regulatory networks from single-cell data is itself a benchmarked problem. The reference evaluation compared inference algorithms against synthetic networks, literature-curated Boolean models and transcriptional networks, and showed that measured accuracy depends strongly on how the evaluation is designed (Pratapa *et al.*, 2020). Subsequent work has added methods rather than settling the question: reviews now catalogue machine-learning approaches (Hegde *et al.*, 2025) and graph neural networks in particular (Jamal Alkhateeb and Awad, 2026), while individual methods such as GeneLink (Zhang *et al.*, 2025a), LogicSR (Zhang *et al.*, 2025b) and scMGATGRN (Yuan *et al.*, 2024) target problems that recur in this literature, including sparsity and over-smoothing. All of it evaluates the inferred network, that is, whether the correct edges are recovered. None of it asks what happens when that network is then used to predict the effect of losing a gene.
...
Finally, the application presented here follows the case-control logic of the first published scTenifoldKnk application in neuroinflammatory disease (Maggi *et al.*, 2023) and uses a published single-nucleus atlas of subcortical multiple sclerosis lesions (Nature Neuroscience, 2024). The candidate panel and the conditions were therefore chosen before any of the analyses reported below.
...
Containment held for **3,191 of 3,195 knockouts (99.87%)**, and it held for **100% (all 3,191) of knockouts with outdegree > 0**. All four violations were knockouts with **outdegree 0**, that is, genes whose entire row in the inferred network is zero, so that "knocking them out" changes nothing: `TSHR` in the Control network (15 significant genes), `ACTA2` and `FAF1` in the MS network (7 each) and `TSHR` in split-half A (9). The bound |DR(*g*)| ≤ outdegree(*g*) + 1 followed directly.

The developers' own published *Trem2* result is consistent with this structure rather than an independent validation of it: *Trem2* has outdegree 2,744 and 128 significant genes, **all 128 of which are direct targets of *Trem2***, and the list is far shorter than the naive |DR| = outdegree + 1 expectation.
...
**R2. Report the outdegree of every knocked-out gene, and exclude outdegree-0 knockouts.** In our data all containment violations, and both of the largest spuriously large lists, came from genes with outdegree 0; for them the knockout is a no-op. Outdegree is trivially available from the adjacency matrix and should be a required element of any reported knockout.
...
3. **Donor composition.** The two conditions share no donors (Control 6 donors, MS 7), and two MS donors contribute most MS cells; any condition effect is therefore inseparable from donor effects, which is one reason we report no biological claim.
...
The four checkpoints and their thresholds were fixed before the replicate runs. Every other analysis reported here, namely the empirical-null panel comparison, the parameter and depth-matched sensitivity analyses, the cross-tool comparison with CellOracle and the target-level weight-share test, was exploratory and is described as such. Larger replicate subsamples (1,000 cells) were added after the first round to separate sample-size effects from genuine instability.
...
All input data are public (GEO: GSE279180; the 10 replication networks are distributed with the `cailab-tamu/scTenifoldKnk` repository). All analysis scripts, parameter files, figure and table files and the documentation are openly available at **https://github.com/zhuzilong1976/zhuzilong** and archived at Zenodo under DOI **10.5281/zenodo.22752977**, which resolves to the version corresponding to this manuscript (**v1.1.0**: 10.5281/zenodo.22814877). The large derived objects (the stability replicate networks, the complete knockout distance matrices and the CellOracle outputs) are regenerated by the released scripts from the public inputs, with measured runtimes given in `code/README.md`; a separate Zenodo data record can be deposited on request.

*Independent reproduction.* Download the archived release (`10.5281/zenodo.22814877`), install the packages listed in `env/README.md`, then run, from the repository root, `Rscript code/03_controls/29_stability_analysis.R` to regenerate Table S10 and Table S11 from the 20 pre-computed replicate objects, and `Rscript code/07_supplementary/67_stability_checkpoints_C3C4.R` to regenerate Tables S12 and S13. This takes under 30 minutes on an 8-core machine; regenerating the 20 replicate networks from the count matrix (`code/03_controls/28_stability_replicates.R`, 10 replicates per condition) takes about 4 hours per condition with 8 workers, and the runtimes of the full all-gene knockout pipeline for the two primary networks are documented in `code/README.md`.
